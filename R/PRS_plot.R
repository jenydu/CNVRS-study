...
fulltable <- read.csv('data/CNVRS_PRS_association_kunkle.csv', header = TRUE)
...
# Melt the data frame
dfm <- melt(df[, c('Phenotype', 'PRS-LOAD only', 'PRS-LOAD + CNV-RS1 + CNV-RS2')], id.vars = 'Phenotype')
...
g<-ggplot(dfm, aes(x = Phenotype, y = value)) +
  ...
  labs(fill = "Category") +
  scale_fill_manual(values = c('PRS-LOAD only' = 'darkgoldenrod1', 'PRS-LOAD + CNV-RS1 + CNV-RS2' = 'skyblue')) +
  theme_bw() +
  ...
  labs(x = "Outcomes") +
  scale_y_continuous(limits = c(0, 0.4), name = "Adjusted R-Squared")
...
cogdx_avg <- data.frame(
  ...
  variable = c('PRS-LOAD only', 'PRS-LOAD + CNV-RS1 + CNV-RS2'),
...
cogdx_avg$variable <- factor(cogdx_avg$variable, levels = c('PRS-LOAD only', 'PRS-LOAD + CNV-RS1 + CNV-RS2'))
...
g2 <- ggplot(cogdx_avg, aes(x = Phenotype, y = value)) +
  ...
  scale_y_continuous(limits = c(0, 0.4), name = "Adjusted McFadden Pseudo R-Squared")
...
combined_plot <- g + g2 +
  plot_layout(ncol = 2, byrow = TRUE, widths = c(9, 2)) +
